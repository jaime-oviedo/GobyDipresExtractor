...
public class BudgetCsvFilesDownloader extends AbstractDownloader {
  ...
  */
  public void download(final List<String> budgetFileList)
    ...
    if (useTimestamp) {
      extractionDestination = new StringBuilder(getDestination()).append("-")
    ...
    BudgetCsvFilesDownloader.log
        .finest(new StringBuilder("Generando directorio:")
            .append(extractionDestination).toString());
    try {
      FileUtils.deleteDirectory(destinationDirectory);
    } catch (final IOException e1) {
      BudgetCsvFilesDownloader.log
          .finest(new StringBuilder("Unable to delete directory:")
    ...
    while (i.hasNext()) {
      ...
      final String sourceFile = new StringBuilder(documentsSource).append('/')
          .append(fileName).toString();
      BudgetCsvFilesDownloader.log
          .finest(new StringBuilder("Se descargará el siguiente archivo:")
      ...
      if (failedFiles.size() > 0) {
        ...
        for (final String string : failedFiles) {
          msg.append(new StringBuilder(string).append("\n")).toString();
        }
  ...
  public List<String> readFileList() throws DownloadException {
    ...
    try {
      while ((l = extractionListFileStream.readLine()) != null) {
        fileList.add(l);
        BudgetCsvFilesDownloader.log.fine(
            new StringBuilder("Added file to list:").append(l).toString());
      }
    } catch (final IOException e) {
